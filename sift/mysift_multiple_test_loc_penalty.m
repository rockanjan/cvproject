...
function [matches] = computematch(test_image, train_image, desc1, desc2, loc1, loc2, distRatio)
    ...
    for k=1:size(desc1, 1)
        dotprods = zeros (size(desc2T, 2)); %match of all features in training to this test
        for j=1:size(desc2, 1)
            %check the location
            locDist = sqrt( (loc1(k, 1) - loc2(j, 1))^2 + (loc1(k, 2) - loc2(j, 2))^2 );
            if(locDist < maxdist * locationRatio)
                dotprods(j) = desc1(k, :) * desc2T(:, j);
            else
                dotprods(j) = 0;
            end
        end
